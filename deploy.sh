...
if [ -z "$PORT" ]; then
    echo -e "\e[1;31mError: Port is required.\e[0m"
    exit 1
fi
...
if [ ! -d "$APP_DIR" ]; then
    echo -e "\e[1;31mError: App directory does not exist: $APP_DIR\e[0m"
    exit 1
fi
...
if [ -z "$SYSTEM_USER" ]; then
    echo -e "\e[1;31mError: Could not find system user for $DOMAIN.\e[0m"
    exit 1
fi
...
echo "\e[1;32mDeployment completed successfully for ${DOMAIN}.\e[0m"
EOF
)
...
if [ $? -eq 0 ]; then
    ...
    echo -e "\e[1;31mError: Failed to set ownership for $DEPLOY_SCRIPT_PATH.\e[0m"
fi

echo -e "\e[1;32mDeploy script created successfully at ${DEPLOY_SCRIPT_PATH}.\e[0m"

# Prompt to add a GitHub repo
read -p "Add a GitHub repo (SSH link / n): " REPO_LINK
if [ "$REPO_LINK" != "n" ] && [ -n "$REPO_LINK" ]; then
    echo ""
    echo -e "\e[1;33mSetting up GitHub repository for ${DOMAIN}, this may take a few minutes...\e[0m"
    ...
    if [ $? -eq 0 ]; then
        ...
        echo -e "\e[1;31mError: Failed to set up GitHub repository.\e[0m"
    fi
else
...
echo -e "\e[1;32mSetup complete for ${DOMAIN}.\e[0m"
echo ""
echo -e "\e[4;33m**If you are using Apache web services, follow these steps**\e[0m"
...
echo ""
echo "Each domain will have its own pm2 process. To view all the currently running processes, use the following command: './check-pm2.sh'"
